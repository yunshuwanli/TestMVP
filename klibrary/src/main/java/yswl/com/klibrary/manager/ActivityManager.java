...
public class ActivityManager {
    ...
    /**
     *
     * 注意：必须在setContentView之前 super.onCreate(savedInstanceState); 之后调用
     * 设置activity 为全屏的
     * @param ac  目标activity
     */
    public static void setFullSceen(Activity ac){
        ac.getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);//去掉信息栏
    }
    /**
     * 注意：必须在setContentView之前 super.onCreate(savedInstanceState); 之后调用
     * 设置activity 为全屏的并且没有标题
     * @param ac  目标activity
     */
    public static void setFullSceenAndNoTitile(Activity ac){
        setFullSceen(ac);
        setNoTitile(ac);
    }
    /**
     ...
     * @param ac  目标activity
     */
    public static void setNoTitile(Activity ac){
        ac.requestWindowFeature(Window.FEATURE_NO_TITLE);//去掉标题栏
    }
}
